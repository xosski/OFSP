...
class OrbitalStationUI(QMainWindow):
    ...
    def _init_backend(self):
        ...
        # Initialize Wraith Engine
        if WraithEngine:
            try:
                self.wraith_engine = WraithEngine.WraithEngine(mode="aggressive")
                print("Wraith Engine initialized")
            except Exception as e:
                print(f"Wraith Engine initialization error: {e}")
                self.wraith_engine = None
        else:
            self.wraith_engine = None
    ...
    def _create_ui(self):
        ...
        self._create_wraith_engine_tab()
        self._create_hades_ai_tab()
        self._create_donation_tab()
    
    def _create_wraith_engine_tab(self):
        """Create Wraith Engine tab for deep process purging and malware removal"""
        widget = QWidget()
        layout = QVBoxLayout(widget)
        
        # Header
        header_frame = QGroupBox("👻 WRAITH Engine - Deep Process Purger")
        ...
        # Status indicators
        self.wraith_status_label = QLabel("Status: " + ("🟢 Active" if self.wraith_engine else "🔴 Not Available"))
        self.wraith_status_label.setStyleSheet("color: #9932CC; font-size: 14px;")
        header_layout.addWidget(self.wraith_status_label)
        
        self.wraith_mode_label = QLabel("Mode: Aggressive")
        self.wraith_mode_label.setStyleSheet("color: #BA55D3; font-size: 12px;")
        header_layout.addWidget(self.wraith_mode_label)
        ...
        # Left side - Process targeting
        target_widget = QWidget()
        target_layout = QVBoxLayout(target_widget)
        
        # Target selection
        target_frame = QGroupBox("🎯 Target Selection")
        target_inner = QVBoxLayout(target_frame)
        
        # PID input
        pid_row = QHBoxLayout()
        pid_row.addWidget(QLabel("Target PID:"))
        self.wraith_pid_input = QLineEdit()
        self.wraith_pid_input.setPlaceholderText("Enter process ID...")
        pid_row.addWidget(self.wraith_pid_input)
        target_inner.addLayout(pid_row)
        
        # Name input
        name_row = QHBoxLayout()
        name_row.addWidget(QLabel("Process Name:"))
        self.wraith_name_input = QLineEdit()
        self.wraith_name_input.setPlaceholderText("e.g., malware.exe")
        name_row.addWidget(self.wraith_name_input)
        target_inner.addLayout(name_row)
        
        target_layout.addWidget(target_frame)
        
        # Actions
        actions_frame = QGroupBox("⚔️ Wraith Actions")
        actions_layout_inner = QGridLayout(actions_frame)
        
        scan_mem_btn = QPushButton("🧠 Scan Memory")
        scan_mem_btn.setStyleSheet("background-color: #8B008B; color: white; font-weight: bold; padding: 10px;")
        scan_mem_btn.clicked.connect(self._wraith_scan_memory)
        actions_layout_inner.addWidget(scan_mem_btn, 0, 0)
        
        scan_all_btn = QPushButton("🌐 Scan All Processes")
        scan_all_btn.setStyleSheet("background-color: #9400D3; color: white; font-weight: bold; padding: 10px;")
        scan_all_btn.clicked.connect(self._wraith_scan_all)
        actions_layout_inner.addWidget(scan_all_btn, 0, 1)
        
        purge_btn = QPushButton("💀 PURGE TARGET")
        purge_btn.setStyleSheet("background-color: #DC143C; color: white; font-weight: bold; padding: 10px;")
        purge_btn.clicked.connect(self._wraith_purge_target)
        actions_layout_inner.addWidget(purge_btn, 1, 0)
        
        quarantine_btn = QPushButton("🔒 Quarantine")
        quarantine_btn.setStyleSheet("background-color: #FF8C00; color: white; font-weight: bold; padding: 10px;")
        quarantine_btn.clicked.connect(self._wraith_quarantine_target)
        actions_layout_inner.addWidget(quarantine_btn, 1, 1)
        
        target_layout.addWidget(actions_frame)
        
        # Process list
        proc_frame = QGroupBox("📋 Running Processes")
        proc_layout = QVBoxLayout(proc_frame)
        
        refresh_proc_btn = QPushButton("🔄 Refresh Process List")
        refresh_proc_btn.clicked.connect(self._wraith_refresh_processes)
        proc_layout.addWidget(refresh_proc_btn)
        
        self.wraith_process_table = QTableWidget()
        self.wraith_process_table.setColumnCount(4)
        self.wraith_process_table.setHorizontalHeaderLabels(["PID", "Name", "Executable", "Memory"])
        self.wraith_process_table.setAlternatingRowColors(True)
        self.wraith_process_table.setSelectionBehavior(QTableWidget.SelectRows)
        self.wraith_process_table.cellDoubleClicked.connect(self._wraith_select_process)
        proc_layout.addWidget(self.wraith_process_table)
        
        target_layout.addWidget(proc_frame)
        ...
        # Scan results
        results_frame = QGroupBox("🔍 Scan Results & Detections")
        results_inner = QVBoxLayout(results_frame)
        
        self.wraith_results_table = QTableWidget()
        self.wraith_results_table.setColumnCount(5)
        self.wraith_results_table.setHorizontalHeaderLabels(["Type", "Address", "Severity", "Description", "Time"])
        self.wraith_results_table.setAlternatingRowColors(True)
        results_inner.addWidget(self.wraith_results_table)
        
        results_layout.addWidget(results_frame)
        
        # Activity log
        log_frame = QGroupBox("📜 Wraith Activity Log")
        log_inner = QVBoxLayout(log_frame)
        
        self.wraith_log_output = QTextEdit()
        self.wraith_log_output.setReadOnly(True)
        self.wraith_log_output.setPlaceholderText("Wraith Engine activity will appear here...")
        self.wraith_log_output.setStyleSheet("background-color: #0d0d0d; color: #9932CC; font-family: Consolas;")
        log_inner.addWidget(self.wraith_log_output)
        
        clear_log_btn = QPushButton("🧹 Clear Log")
        clear_log_btn.clicked.connect(lambda: self.wraith_log_output.clear())
        log_inner.addWidget(clear_log_btn)
        
        results_layout.addWidget(log_frame)
        splitter.addWidget(results_widget)
        
        layout.addWidget(splitter)
        
        # Progress bar
        self.wraith_progress = QProgressBar()
        self.wraith_progress.setStyleSheet("""
            QProgressBar {
                border: 2px solid #9932CC;
                border-radius: 5px;
                text-align: center;
            ...
            QProgressBar::chunk {
                background: qlineargradient(x1: 0, y1: 0, x2: 1, y2: 0,
                                          stop: 0 #8B008B, stop: 1 #9932CC);
            }
        """)
        layout.addWidget(self.wraith_progress)
        
        self.tabs.addTab(widget, "👻 Wraith Engine")
        
        # Load initial process list
        QTimer.singleShot(1000, self._wraith_refresh_processes)
    
    def _wraith_log(self, message: str):
        """Add message to Wraith log"""
        timestamp = datetime.now().strftime("%H:%M:%S")
        self.wraith_log_output.append(f"[{timestamp}] {message}")
    
    def _wraith_scan_memory(self):
        """Scan target process memory"""
        if not self.wraith_engine:
            QMessageBox.warning(self, "Error", "Wraith Engine not available")
            return
        
        pid_text = self.wraith_pid_input.text().strip()
        if not pid_text:
            QMessageBox.warning(self, "Error", "Please enter a PID to scan")
            return
        
        try:
            pid = int(pid_text)
        except ValueError:
            QMessageBox.warning(self, "Error", "Invalid PID format")
            return
        
        self._wraith_log(f"Starting memory scan for PID {pid}...")
        self.wraith_progress.setValue(0)
        
        def progress_callback(percent, message):
            self.wraith_progress.setValue(percent)
            self._wraith_log(message)
        
        self.wraith_engine.set_progress_callback(progress_callback)
        
        try:
            detections = self.wraith_engine.scan_memory(pid)
            self._wraith_display_results(detections)
            self._wraith_log(f"Scan complete. Found {len(detections)} detections.")
        except Exception as e:
            self._wraith_log(f"Scan error: {str(e)}")
            QMessageBox.critical(self, "Scan Error", str(e))
    
    def _wraith_scan_all(self):
        """Scan all running processes"""
        if not self.wraith_engine:
            QMessageBox.warning(self, "Error", "Wraith Engine not available")
            return
        
        self._wraith_log("Starting full system scan...")
        self.wraith_progress.setValue(0)
        ...
        try:
            detections = self.wraith_engine.scan_all_processes()
            self._wraith_display_results(detections)
            self._wraith_log(f"Full scan complete. Found {len(detections)} detections.")
        except Exception as e:
            self._wraith_log(f"Scan error: {str(e)}")
    
    def _wraith_purge_target(self):
        """Purge the target process"""
        if not self.wraith_engine:
            QMessageBox.warning(self, "Error", "Wraith Engine not available")
            return
        
        pid_text = self.wraith_pid_input.text().strip()
        name_text = self.wraith_name_input.text().strip()
        
        if not pid_text and not name_text:
            QMessageBox.warning(self, "Error", "Please enter a PID or process name")
            return
        
        # Confirm purge action
        target = f"PID {pid_text}" if pid_text else f"'{name_text}'"
        reply = QMessageBox.question(
            self, "Confirm Purge",
            f"⚠️ WARNING: This will PERMANENTLY DELETE {target} and all associated files!\n\n"
        ...
        self._wraith_log(f"Initiating PURGE on {target}...")
        
        try:
            pid = int(pid_text) if pid_text else None
            name = name_text if name_text else None
            
            result = self.wraith_engine.purge(name=name, pid=pid)
            
            if result['success']:
                self._wraith_log("✅ PURGE COMPLETE")
                for action in result['actions']:
                    self._wraith_log(f"  ✓ {action}")
                QMessageBox.information(self, "Purge Complete", 
                    f"Target purged successfully!\n\nActions taken:\n" + 
                    "\n".join(result['actions']))
            else:
                self._wraith_log("❌ PURGE FAILED")
                for error in result['errors']:
                    self._wraith_log(f"  ✗ {error}")
                QMessageBox.warning(self, "Purge Failed", 
                    "Errors occurred:\n" + "\n".join(result['errors']))
                    
        except Exception as e:
            self._wraith_log(f"Purge error: {str(e)}")
            QMessageBox.critical(self, "Purge Error", str(e))
        
        self._wraith_refresh_processes()
    
    def _wraith_quarantine_target(self):
        """Quarantine the target process executable"""
        if not self.wraith_engine:
            QMessageBox.warning(self, "Error", "Wraith Engine not available")
            return
        ...
        proc = self.wraith_engine.resolve_process(name=name, pid=pid)
        if not proc:
        ...
        try:
            exe_path = proc.exe()
            quarantine_path = self.wraith_engine.quarantine_file(exe_path, str(self.quarantine_dir))
            if quarantine_path:
                proc.kill()
                self._wraith_log(f"Process quarantined: {exe_path} -> {quarantine_path}")
                QMessageBox.information(self, "Quarantined", f"File quarantined to:\n{quarantine_path}")
            else:
                QMessageBox.warning(self, "Error", "Failed to quarantine file")
        except Exception as e:
            self._wraith_log(f"Quarantine error: {str(e)}")
            QMessageBox.critical(self, "Quarantine Error", str(e))
        
        self._wraith_refresh_processes()
    
    def _wraith_refresh_processes(self):
        """Refresh the process list"""
        if not self.wraith_engine:
            return
        
        try:
            processes = self.wraith_engine.get_process_list()
            self.wraith_process_table.setRowCount(len(processes))
            
            for i, proc in enumerate(processes):
                self.wraith_process_table.setItem(i, 0, QTableWidgetItem(str(proc['pid'])))
                self.wraith_process_table.setItem(i, 1, QTableWidgetItem(proc['name']))
                self.wraith_process_table.setItem(i, 2, QTableWidgetItem(proc.get('exe', '') or 'N/A'))
                
                mem_info = proc.get('memory')
                if mem_info and hasattr(mem_info, 'rss'):
                    mem_mb = mem_info.rss / (1024 * 1024)
                    self.wraith_process_table.setItem(i, 3, QTableWidgetItem(f"{mem_mb:.1f} MB"))
                else:
                    self.wraith_process_table.setItem(i, 3, QTableWidgetItem("N/A"))
            
            self.wraith_process_table.resizeColumnsToContents()
        except Exception as e:
            self._wraith_log(f"Error refreshing processes: {str(e)}")
    
    def _wraith_select_process(self, row, col):
        """Handle double-click on process row"""
        pid_item = self.wraith_process_table.item(row, 0)
        name_item = self.wraith_process_table.item(row, 1)
        
        if pid_item:
            self.wraith_pid_input.setText(pid_item.text())
        if name_item:
            self.wraith_name_input.setText(name_item.text())
    
    def _wraith_display_results(self, detections):
        """Display scan results in the table"""
        self.wraith_results_table.setRowCount(len(detections))
        
        for i, det in enumerate(detections):
            self.wraith_results_table.setItem(i, 0, QTableWidgetItem(det.get('type', 'Unknown')))
            self.wraith_results_table.setItem(i, 1, QTableWidgetItem(det.get('address', 'N/A')))
            
            severity = det.get('severity', 'Unknown')
            severity_item = QTableWidgetItem(severity)
            if severity == 'Critical':
                severity_item.setBackground(QColor(139, 0, 0))
            elif severity == 'High':
                severity_item.setBackground(QColor(178, 34, 34))
            self.wraith_results_table.setItem(i, 2, severity_item)
            
            self.wraith_results_table.setItem(i, 3, QTableWidgetItem(det.get('description', '')))
            self.wraith_results_table.setItem(i, 4, QTableWidgetItem(det.get('timestamp', '')))
        
        self.wraith_results_table.resizeColumnsToContents()
